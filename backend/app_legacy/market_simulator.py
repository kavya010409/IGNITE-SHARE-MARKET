...
async def process_market_tick(redis_client: aioredis.Redis) -> None:
    async with AsyncSessionFactory() as session:
        async with session.begin():
            ...
            for news in unapplied_news:
                ...
                if elapsed_seconds >= NEWS_SHOCK_DELAY_SECONDS:
                    ...
                    logger.info(
                        f"⏱️ News '{news.headline[:25]}...' in 2-min anticipation window ({elapsed_seconds:.0f}s / 120s). Ticking organically."
                    )
            ...
            for stock in stocks:
                ...
                if effective_mult != 1.0:
                    # Force massive price realignment calculation (e.g., +25% or -30% shift)
                    shock_shift = (effective_mult - 1.0)
                    final_change_pct = shock_shift + base_noise
                else:
                    # Restricted strictly to basic organic noise (-0.4% to +0.4%)
        ...
        # Broadcast Real-time Ticks to Redis Channel
        payload = {
            "event": "market_tick",
        ...
        logger.info(f"Market tick published for {len(stocks)} stocks.")


async def start_market_simulator() -> None:
    logger.info("Initializing Continuous 15-Second Market Simulator with 2-Minute News Shock Execution Engine...")
